...
- Пройдены тесты по критериям приёмки User Stories (проверка ролевой модели: admin, manager, engineer, observer).
- Проведено нагрузочное тестирование (целевой отклик ≤ 1 сек. под типовой нагрузкой).

### Как запускать тесты

Юнит-тесты и интеграционные тесты находятся в `backend/tests/`.
...
Используется Locust (`backend/tests/locustfile.py`).
...
Цель: p95 отклика ≤ 1 сек. при типовой нагрузке (параллельные логины, чтение проектов/дефектов). Настройте количество пользователей и скорость спауна в UI Locust, проверьте метрики и при необходимости увеличьте ресурсы.
